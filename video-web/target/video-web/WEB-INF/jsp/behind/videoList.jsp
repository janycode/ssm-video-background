...
<%@ taglib prefix="abyg" uri="http://abyg.com.cn" %>
<!-- 分页插件 -->
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    <script type="text/javascript">
        function showAddPage() {
            location.href = "${pageContext.request.contextPath}/video/addAndUpdVideo";
        }
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse"
            ...
            <p class="navbar-text navbar-right">
                欢迎&nbsp;<span style="color: orange">${loginAdmin.username}</span>&nbsp;&nbsp;
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <form id="formVideoList" class="navbar-form navbar-right" action="${pageContext.request.contextPath}/video/list" method="post">
                ...
                <input type="text" name="title" id="videoTitle" class="form-control" placeholder="标题" value="${queryVo.title}">
                <div class="btn-group">
                ...
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                        <li value="">
                            <a href="#" onclick="showName(this,'',2)">--请选择课程--</a>
                        </li>
                        <c:forEach items="${courseList}" var="course">
                            <li value="${course.id}">
        ...
        <table class="table table-bordered table-hover"
            ...
            <c:forEach items="${pageInfo.list}" var="video" varStatus="status">
                <tr>
                    ...
                    <td>${video.course.courseTitle}</td>
                    <td>${video.time}</td>
                    <td>${video.playNum}</td>
                    <td>
                        <a href="${pageContext.request.contextPath}/video/addAndUpdVideo?id=${video.id}">
                            <span class="glyphicon glyphicon glyphicon-edit" aria-hidden="true"></span>
                        </a>
                        <!-- js中如果使用el表达式，请用单引号包括，避免造成一些语法问题 -->
                        <a onclick="return delVideoById(this,'${video.id}','${video.title}')">
...
<%--        <abyg:page url="${pageContext.request.contextPath}/video/list"></abyg:page>--%>
<%--    </div>--%>
<%--</div>--%>
<div class="container">
...
<script type="text/javascript">
    ...
    function jumpPage(pageNum) {
        // JS 解决选中条件后并未查询，但跳转其他页数会自动查询的问题
        $("#videoTitle").val('${queryVo.title}');
        $("#speakerId").val('${queryVo.speakerId}');
        $("#courseId").val('${queryVo.courseId}');

        // 设置当前页数到 from 表单的隐藏域，统一控制器访问路径为1个
        $("#pageNum").val(pageNum);
